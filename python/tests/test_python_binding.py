...
def test_product_is_mojo_030_not_stale_python_010() -> None:
    ...
    assert 'version = "0.3.1"' in pixi
    assert '__version__ = "0.3.1"' in init_py
    assert 'TAKT_VERSION = "0.3.1"' in init_mojo
    assert "**Version 0.3.1**" in readme
...
def test_product_stamps_are_030_not_leftover_mojo_only_020() -> None:
    """#25: main README/pixi must match the 0.3.1 + python/takt tree, not v0.2.0."""
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    pixi = (ROOT / "pixi.toml").read_text(encoding="utf-8")
    pyproject = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    ...
    assert "takt-0.3.1.tar.gz" in readme
